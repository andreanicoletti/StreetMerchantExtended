import {Link, SearchProduct} from './model';
import {config} from '../config';
import {isLink} from './search-page';
 ...
 *
 * @param item The store link of the GPU
 */
export function filterStoreItem(item: Link | SearchProduct): boolean {
  if (isLink(item)) {
    return (
      filterBrand(item.brand) &&
      filterModel(item.model, item.series) &&
      filterSeries(item.series)
    );
  } else {
    return filterSeries(item.series);
  }
}

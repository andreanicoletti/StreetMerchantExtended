...
import {Link, Store, getStores, SearchProduct} from './model';
...
import {filterStoreItem} from './filter';
...
import { isLink, isSearchProduct, getInStockSearchProducts} from './search-page';
...
async function lookup(browser: Browser, store: Store) {
  ...
  const returnLookup = (store.searchPage === undefined) || (store.searchPage.products.length === 0)
  if (config.store.useSearchPage && returnLookup) {
    return;
  }

  if (store.linksBuilder && !config.store.useSearchPage) {
  ...
  const items = config.store.useSearchPage ? store.searchPage?.products : store.links
  if (items === undefined) {
    return;
  }
  ...
  for (const item of items) {
    if (!filterStoreItem(item)) {
      continue;
    }

    const fakeLinkProduct: Link = {
      model: '',
      series: item.series,
      brand: '',
      url: isLink(item) ? item.url : item.searchPageUrl,
    }
    if (isLink(item) && config.page.inStockWaitTime && inStock[item.url]) {
      logger.info(Print.inStockWaiting(item, store, true));
      continue;
    }
    ...
    try {
      if (isLink(item)) {
        statusCode = await lookupIem(browser, store, page, item);
      } else {
        statusCode = await lookupSearch(browser, store, page, item);
      }
    } catch (error: unknown) {
      const logItem = isLink(item) ? item : fakeLinkProduct;
      if (store.currentProxyIndex !== undefined && store.proxyList) {
        const proxy = `${store.currentProxyIndex + 1}/${
          store.proxyList.length
        }`;
        logger.error(
          `✖ [${proxy}] [${store.name}] ${logItem.brand} ${logItem.series} ${
            logItem.model
          } - ${(error as Error).message}`
        );
      } else {
        logger.error(
          `✖ [${store.name}] ${logItem.brand} ${logItem.series} ${logItem.model} - ${
            (error as Error).message
          }`
        );
      }
    ...
    await processBackoffDelay(store, fakeLinkProduct, statusCode);
...
export async function lookupSearch(
  ...
  const givenWaitFor = store.waitUntil ? store.waitUntil : 'networkidle0';
  const response: HTTPResponse | null = await page.goto(product.searchPageUrl, {
    waitUntil: givenWaitFor,
  });
  ...
  const fakeLinkProduct: Link = {
    ...
    brand: '',
    url: product.searchPageUrl,
  }
  let foundResult = false;

  const successStatusCodes = store.successStatusCodes ?? [[0, 399]];
  const statusCode = await handleResponse(browser, store, page, fakeLinkProduct, response);

  if (!isStatusCodeInRange(statusCode, successStatusCodes)) {
    return statusCode;
  }

  const storeUrls = store.links.map(link => link.url);
  const searchInStock = await getInStockSearchProducts(page, store);
  for (const {url, outOfStock, price} of searchInStock) {
    if (!storeUrls.includes(url)) {
      continue;
    }
    const linkFromStoreUrls = store.links.find((link) => link.url === url);
    if (linkFromStoreUrls === undefined) {
      continue;
    }

    const itemLink: Link = {
      brand: linkFromStoreUrls.brand,
      model: linkFromStoreUrls.model,
      series: linkFromStoreUrls.series,
      url: url,
      price: price === 0 ? null : price
    };

    if (config.page.inStockWaitTime && inStock[url]) {
      logger.info(Print.inStockWaiting(itemLink, store, true));
      continue;
    }

    if (outOfStock) {
      logger.info(Print.outOfStock(itemLink, store, true));
      continue;
    }

    logger.info(`${Print.inStock(itemLink, store, true)}\n${url}`);

    if (config.browser.open) {
      const openCart = store.links.find(link => (link.url === url))?.openCartAction;
      await (openCart === undefined ? open(url) : openCart(browser));
    }
  ...
  if (config.page.inStockWaitTime) {
    ...
    sendNotification(itemLink, store);
    foundResult = true;
  }

  if (config.page.screenshot && foundResult) {
    logger.debug('ℹ saving screenshot');

    const screenshot = `success-${Date.now()}.png`;
...
async function handleResponse(
  ...
  if (!response) {
    logger.debug(Print.noResponse(item, store, true));
  }
  ...
  if (!isStatusCodeInRange(statusCode, successStatusCodes)) {
    if (statusCode === 429) {
      logger.warn(Print.rateLimit(item, store, true));
    } else if (statusCode === 503) {
      if (await checkIsCloudflare(store, page, item)) {
        if (recursionDepth > 4) {
          logger.warn(Print.recursionLimit(item, store, true));
        } else {
        ...
        logger.warn(Print.badStatusCode(item, store, statusCode, true));
      }
    } else {
      logger.warn(Print.badStatusCode(item, store, statusCode, true));
    }
